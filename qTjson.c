...
//==
typedef struct{
    struct qTjson *child; //pointer be bacheyye har object
    char type[50]; //esme itemi ke intooe
    char content[200]; //mohtavaye in item
} qTjson;


void qTjson_addItemToObject(qTjson **root,char type_inp[],char content_inp[])
{
    qTjson *new_child =(qTjson*)malloc(sizeof(qTjson));
    qTjson *last = *root;

    strcpy(new_child->type,type_inp);
    strcpy(new_child->content,content_inp);
...
char* qTjson_getObjectItem(qTjson *object,char itemName[])
{
    ...
    while(object->child != NULL){
        if(strcmp(object->type,itemName)==0) return(object->content);
        object=object->child;
    }

    if(strcmp(object->type,itemName)==0)
...
qTjson *qTjson_deleteObjectItem(qTjson *root,char itemName[])
{
    ...
    if(strcmp(itemName,root->type)==0){
        qTjson *tmp;
        tmp=root->child;
        free(root);
        return(tmp);
    }
   root->child = qTjson_deleteObjectItem(root->child,itemName);
   return (root);
}
